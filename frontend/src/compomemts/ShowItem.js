...
export default function ShowItem({doc_id , setshow}) {
  ...
  return (
    <div>
        <h3 className='showItemText text-center'>{item?.name}</h3>
        <div className='img-container'><img className="card-img-top" src={item?.image} alt="تصویری برای این سرویس در دسترس نیست" /></div>

        <p className="h5"><span className='showItemText'>Dr.{item?.last_name}</span> <span className='showItemTitles'>: پزشک</span></p>
