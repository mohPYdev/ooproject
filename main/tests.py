...
class ShiftViewSetTestCase(TestCase):
    ...
    def test_retrieve_shift(self):
        shift = Shift.objects.create(start_date=datetime.datetime.today(), end_date=datetime.datetime.today())
...
class ReservationViewSetTestCase(TestCase):
    ...
    def test_retrieve_reservation(self):
        reservation = Reservation.objects.create(time_date=datetime.datetime.now(), reserver_id="1")
    ...
    def test_create_reservation(self):
        url = reverse('reservation-list')
        reserver = User.objects.create(username='testuser2')
        service = Service.objects.create(name='Test Service', price=22.3)
        item = Item.objects.create(name='Test Service', description='test')
        shift = Shift.objects.create(start_date=datetime.datetime.now(), end_date=datetime.datetime.now())

        data = {
            'reserver': reserver.pk,
            'time_date': datetime.datetime.now().isoformat(),
            'service': service.pk,
            'shift': shift.pk,
